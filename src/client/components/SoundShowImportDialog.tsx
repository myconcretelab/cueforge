...
export function SoundShowImportDialog({ onClose, onImported }: Props) {
  ...
  return <div className="dialog-backdrop" onMouseDown={(event) => !busy && event.target === event.currentTarget && onClose()}>
    <section className="dialog soundshow-dialog">
      ...
      {!analysis && !result && <>
        <div className="import-explainer"><FileArchive size={34} /><div><strong>Sélectionnez le dossier de votre projet</strong><p>SonoRiva trouvera le fichier `.ssp`, les sous-dossiers et tous les médias disponibles.</p></div></div>
