...
export function FreesoundDialog({ initialQuery = '', projectId, categories, defaultCategoryId, nextPosition, onImported, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('cueforge:stop-temporary-audio', stop);
    return () => {
      window.removeEventListener('cueforge:stop-temporary-audio', stop);
