...
export function WhatsNewDialog({ releases, currentVersion, onClose }: Props) {
  return <div className="dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="dialog whats-new-dialog" aria-labelledby="whats-new-title">
      <header><div><p className="eyebrow"><PartyPopper size={15} /> S1 · Version {currentVersion}</p><h2 id="whats-new-title">Nouveautés</h2></div><button className="icon-button" onClick={onClose} aria-label="Fermer les nouveautés"><X /></button></header>
